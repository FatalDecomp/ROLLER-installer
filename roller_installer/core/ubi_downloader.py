    ...
    def download(
        ...
        try:
            ...
            if not binary_path.exists():
                # On Windows, ubi might add .exe
                binary_path = install_dir / f"{exe_name}.exe"
            
            if not binary_path.exists():
                raise RuntimeError(f"Downloaded binary not found at expected location: {binary_path}")
            
            logger.info(f"Successfully downloaded ROLLER to: {binary_path}")
            
            if progress_callback:
                progress_callback(f"Downloaded ROLLER {tag} successfully")
